...
- **Critical / important fixes** that change observable behavior (security, data-loss, correctness, fail-closed defaults, API contracts) → update. Reviewer-driven changes are more likely to alter behavior than self-review fixes, so this rule is intentionally broader than `hero-skills:review`'s.
