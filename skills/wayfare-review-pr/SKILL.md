...
if [ -z "$PR_JSON" ] || [ "$PR_JSON" = "null" ]; then
  echo "No PR found for branch '$BRANCH'. Run wayfare:wayfare-push-pr first."
  exit 1
fi
...
**Skip this step entirely when `$NO_MARK_READY` is `true`** (caller passed `--no-mark-ready`, typically `wayfare:wayfare-one-shot` whose own Step 6 owns the mark-ready gate). In that case, jump straight to Step 10. The summary will show `PR state: Draft (mark-ready deferred to caller)`.
...
- **Marked ready, agent configured**: print `Waiting on {agent}'s first review, then run wayfare:wayfare-respond-to-comments.` (no prompt, nothing to invoke yet).
