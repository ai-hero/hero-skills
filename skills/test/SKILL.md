...
name: test
# prettier-ignore
description: Verify an implementation. Runs lint, typecheck, and unit tests on changed files, then detects project type (CLI, backend, frontend, MCP) and runs smoke tests. Use after hero-skills:plan implements.
...
If `HERO.md` is missing, suggest `hero-skills:init` but proceed with auto-detection below.
...
```
hero-skills:test                              # Verify + smoke test everything (default)
hero-skills:test verify                       # Only run lint, typecheck, unit tests
hero-skills:test smoke                        # Only run smoke tests (skip verification)
hero-skills:test backend                      # Only smoke-test the API
hero-skills:test frontend test the login form # Smoke test a specific UI flow
hero-skills:test mcp                          # Smoke test MCP server via Inspector
hero-skills:test cli run the export command   # Smoke test a specific CLI command
```
