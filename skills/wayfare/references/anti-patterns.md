...
- **A goal is runnable** (`active`, or `accepted` with its goal deps `done` and its members all planned): `Next step: hero-skills:wayfare next, to authorize its permissions and run it`; `hero-skills:wayfare do GOAL_ID` is one turn of it.
