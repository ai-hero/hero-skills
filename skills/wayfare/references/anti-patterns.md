...
| Widening a member task's `source:` from inside a turn | The admission test bounds on those paths *because* they were fixed at plan time and read aloud at the gate. A build that moves them picks its own bound. Record the extra files in `## Comments` and leave the field alone. |
...
| A goal table listing only the built items | Then it is a commit log. Every item in `parent` gets a row; the unbuilt ones are how the table says where this is, not just what was done. |
