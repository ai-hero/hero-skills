...
Next step: hero-skills:one-shot $ARGUMENTS, which runs Steps 1-10 in one go.
...
- For pr-review-toolkit: the script looks in `~/.claude/plugins/pr-review-toolkit/` and `$ROOT/.claude/plugins/pr-review-toolkit/`. If the plugin lives elsewhere on the user's setup, the check may report a false `[WARN]`. That is safe, because the worst case is `hero-skills:review-pr` falling back to a thinner review.
