...
```
No HERO.md found. Run hero-skills:init first to configure the project.
```
...
[??] linear (mcp__linear): listed in HERO.md
     ...
     → This is needed for hero-skills:plan to manage issues
...
Run hero-skills:init --update if the project setup has changed.
```
...
- **Idempotent.** Running `hero-skills:setup` twice should be safe — skip what's already done.
- **Reference HERO.md.** Every check should tie back to why it's needed: "Required by HERO.md for hero-skills:push" or "Used by CI (GitHub Actions)".

## Examples

```
hero-skills:setup          # Full guided setup
hero-skills:setup --check  # Just verify, don't change anything
```
