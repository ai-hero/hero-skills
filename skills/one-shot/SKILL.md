...
`one-shot: running recalibrate`, then follow the four phases in
...
- **It doesn't** (an incidental refactor) → still a `kind: feature` (or `architecture`), `origin: one-shot`, `discovered_from: PARENT_ID`, `status: todo`, with `source` set and `target`/`target_ref` absent — there is no plain shape. An incidental **defect** in this repo's own code is `kind: bug` with the same provenance fields and Observed / Expected / Repro in its `## Context`.
- **The defect is in a sibling repo's code** (a registry component, a shared workflow, a library this repo consumes) → it is not this repo's item at all. Deposit a `type: bug` message into that repo's `.plans/inbox/` per `docs/MESSAGES.md` — Observed, Expected, Repro, Where hit, `about:` this item, `awaited: false` — after confirming the destination is a FLEET.md row and showing the draft. Then continue here with a workaround if one exists, or STOP per rule 4 if the fix is a prerequisite (the dependency is on the sibling's answer, so this item goes `suspended` with `awaiting:` the message id and `awaited: true` on the message). Editing the sibling is the one thing this branch never does. `todo` on a build kind means backlog, never READY, so the child waits for planning and the ready-mark like anything else.
