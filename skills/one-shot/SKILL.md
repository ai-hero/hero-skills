...
- **Work the subtasks in order** — a `kind: feature` item carries an ordered `## Subtasks` checklist (written when the feature was planned via think-it-through); implement top to bottom and check each line off (`- [ ]` → `- [x]`) as it completes, so a session that dies mid-flight shows exactly where it stopped. Default to shipping the whole checklist as this one PR; split at a subtask boundary into sequential PRs only when the repo's conventions or reviewable-size norms call for it — then run the pipeline per PR, and the feature stays unfinished until the last one merges (Step 9a).
