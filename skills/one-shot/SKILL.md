...
5. **Order the writes, and define the undo.** Write the child first, verify it exists on disk with its allocated id, then mutate the parent, then append the parent's `## Comments` entry. Reverse that order and an interrupted run loses the work entirely. "Undo it" means: delete the child **by path**, restore the parent's removed lines verbatim, and quote those lines in the hand-back message — an abandoned child left on disk claims target ground the parent is still delivering, and `sync` will believe it.
...
**Record the outcome in `## Comments`, whichever way it goes** — `2026-07-25: DoD line "…" fails (see Design Feedback 2026-07-24); user accepted the divergence and closed out` or `…user declined close-out`. Terminal output is not a record. Without it, a feature the user *deliberately* left open and one whose close-out was simply missed sit in byte-identical state (`reviewing`, PR merged), and `wayfare next` tier 2 reads that state as an oversight — so the next run re-asks a question already answered, and "an accepted divergence is a decision the user makes once" becomes the argument for granting it. An accepted divergence leaves the DoD line unticked with an inline `accepted YYYY-MM-DD, see Design Feedback` annotation, plus the comment; a self-granted one is how a roadmap starts claiming coverage it does not have.
