...
If `HERO.md` is missing, suggest `hero-skills:init` but proceed with defaults (Linear MCP, conventional branches).
...
Options:
1. Run hero-skills:commit to commit first (recommended)
...
- **No commits, no pushes** — Implementation only writes code. `hero-skills:commit` and `hero-skills:push` handle git operations.

Verification (running tests, linters, type checkers, smoke tests) is not part of this skill. Once implementation is complete, the user runs `hero-skills:test` to verify the changes work end-to-end.
...
Next steps:
  hero-skills:test     # Verify the implementation works
  hero-skills:commit   # Review and commit
  hero-skills:push     # Push and open a draft PR
```

## Integration

```
hero-skills:plan PROJ-123              # Plan from a Linear issue, then implement
hero-skills:plan add dark mode toggle  # Plan from a description, then implement
# ... user approves plan in Plan Mode, implementation runs ...
hero-skills:test                       # Verify the implementation
hero-skills:commit                     # Review and commit
hero-skills:push                       # Push and open a draft PR
hero-skills:review                # Run an automated review and address findings
```
...
- Include testing approach in every plan, but actual verification happens in `hero-skills:test`
- Do NOT commit or push from this skill — that's `hero-skills:commit` and `hero-skills:push`
