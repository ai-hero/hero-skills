...
name: push
# prettier-ignore
description: Push current work to remote and open a draft PR by default, so the author can run hero-skills:review before human review. Pass `ready` for a non-draft PR, or a branch name to merge into a target.
...
Push your current work to the remote repository and open a **draft PR by default**. Drafts are the default because the author should run `hero-skills:review` (which calls all pr-review-toolkit agents, applies fixes, and asks for confirmation) before promoting the PR to ready-for-review.
...
Options:
1. Run hero-skills:commit to review and commit first (recommended)
...
Next steps:
  hero-skills:review   # Run automated review and fix findings before marking ready
```
