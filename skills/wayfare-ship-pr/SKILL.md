...
- `.github/workflows/auto-approve.yaml` (or `.yml`) present on the default branch. Run `wayfare:wayfare-hero init recalibrate` to install it
...
Fix:
  1. Run wayfare:wayfare-hero init recalibrate to install the workflow
...
#### Step 7b: Reset to the Default Branch (the merged-branch counterpart to `wayfare:wayfare-hero drop`)

After a successful merge, leave the user on the default branch, pulled, with the merged PR branch cleaned up. We are usually still on the just-merged head. `wayfare-hero drop` mirrors this for the never-merged case; not shared logic.
