...
Run by `wayfare:wayfare-hero init`. Deeply investigate the repository,
...
This was `wayfare:wayfare-hero init`. It is the same procedure; it is reached
...
## Tech Stack
<!-- Auto-managed by wayfare:wayfare-hero init. See HERO.md for full configuration. -->
See [HERO.md](./HERO.md) for the full tech stack configuration detected by `wayfare:wayfare-hero init`.
...
- If a section exists but does **not** reference `HERO.md`, add:
  `See [HERO.md](./HERO.md) for details managed by wayfare:wayfare-hero init.`
...
### Group 0: "Your coding agent" (`wayfare:wayfare-hero init recalibrate`, `wayfare:wayfare-setup-dev`)

- Coding agent (Claude Code, Cursor, Windsurf, etc.)
- Whether hooks/pre-commit integration is possible

Do NOT offer to install a pre-commit hook for `wayfare:wayfare-hero init recalibrate`. Skills surface a stale-HERO.md hint on demand instead; see `scripts/check-hero-staleness.sh`.
...
```markdown
# Hero Configuration
<!-- Generated by wayfare:wayfare-hero init. Re-run with wayfare:wayfare-hero init recalibrate to refresh. -->
...
<!-- HERO.md is refreshed on demand by `wayfare:wayfare-hero init recalibrate`.
...
How your hero skills will use this:
  ...
  wayfare:wayfare-hero init recalibrate → re-investigate and refresh HERO.md on demand (run when project config changes)
...
Only stage the workflow file when Step 6a reported the file is in sync with the plugin (`INSTALL_OK=true`, covering both the fresh-install and already-up-to-date paths). When the installer returned exit 2 (`EXISTS`, drift detected), the working file is still the user's original, and committing it now would falsely claim `wayfare:wayfare-hero init` installed the new version.
...
git commit -m "chore: initialize HERO.md and update AGENTS.md via wayfare:wayfare-hero init"
```
...
```
wayfare:wayfare-hero init              # Investigate, write HERO.md and PLAN.md
wayfare:wayfare-hero init recalibrate  # Re-investigate and update existing config
```
