...
| Skill | Needs |
|-------|-------|
| `hero-skills:commit` | Commit convention, pre-commit run command, issue prefix |
| `hero-skills:push` | Default branch, branch convention, hosting platform (`gh`/`glab`), issue prefix |
| `hero-skills:plan` | PM tool + MCP server name, branch template, issue prefix, project list |
| `hero-skills:test` | Language, framework, lint/format/typecheck commands, test/dev/install commands, ports, dependency file |
| `hero-skills:review` | Code Quality (pre-commit), Code Review Agent (bot username — to dedupe its comments) |
| `hero-skills:check` | CI platform, workflow names, registry, required status checks |
| `hero-skills:check cluster` | Deployment platform, namespaces, ArgoCD, health check endpoints |
| `hero-skills:scan` | Registry, language/framework, dependency files per project |
| `hero-skills:architect` | Repo type, project list, deployment platform |
| `hero-skills:create` | Repo type, coding conventions, code quality tools, project scaffold patterns |
| `hero-skills:setup` | Required tools, recommended tools, MCP servers |
| `hero-skills:respond` | Code Review Agent (agent, trigger, poll-method, bot-username) |
| `hero-skills:ship` | CI/CD (auto-approve workflow installed on default branch), Repository (default branch) |
| `hero-skills:init --update` | All sections — keeps HERO.md in sync via pre-commit |
| `hero-skills:audit` | (internal) Plugin structure validation |
...
## Tech Stack
<!-- Auto-managed by hero-skills:init. See HERO.md for full configuration. -->
See [HERO.md](./HERO.md) for the full tech stack configuration detected by `hero-skills:init`.
...
- If a section exists but does **not** reference `HERO.md`, add a reference line:
  `See [HERO.md](./HERO.md) for details managed by hero-skills:init.`
...
# Also fetch deleteBranchOnMerge — when false, hero-skills:ship will
...
- `deleteBranchOnMerge` → `auto-delete-branches` field. If true, GitHub already deletes merged branches and `hero-skills:ship` will skip cleanup. If false, the skill will delete the remote and local branch after a successful merge unless `auto-delete-branches: false` overrides it in HERO.md.
...
- Whether `.github/workflows/auto-approve.yml` already exists — needed by `hero-skills:ship`
...
- These go into HERO.md `## Developer Setup` as team-shared requirements — individual installation/auth is handled by `hero-skills:setup`
...
- **Dependency file** per project (pyproject.toml, package.json, go.mod, etc.) — needed by `hero-skills:scan` and `hero-skills:test`
- **Lock file** → identifies the package manager (pnpm-lock.yaml → pnpm, yarn.lock → yarn, etc.)
- **Install command** (e.g., `uv sync`, `pnpm install`) — needed by `hero-skills:test` before running
...
- **Exact lint/format/typecheck commands** — not just tool names; `hero-skills:test` needs runnable commands for verification
...
[CONFIRMED] SETTING: VALUE
  Evidence: EVIDENCE
  Used by: hero-skills:commit, hero-skills:push

[NEEDS CONFIRMATION] SETTING: BEST_GUESS
  Evidence: EVIDENCE (and why ambiguous)
  Question: QUESTION
  Used by: hero-skills:plan

[NOT DETECTED] SETTING
  Looked for: WHAT_WAS_CHECKED
  Question: QUESTION
  Used by: hero-skills:check cluster
```

**Group findings into these categories, presented in this order:**

#### Group 0: "Your coding agent" (`hero-skills:init --update`, `hero-skills:setup`)

- Coding agent (Claude Code, Cursor, Windsurf, etc.)
- Whether hooks/pre-commit integration is possible
- Whether to set up `hero-skills:init --update` as a pre-commit hook to keep HERO.md in sync

#### Group 1: "For committing and pushing code" (`hero-skills:commit`, `hero-skills:push`, `hero-skills:ship`)
...
- Whether GitHub auto-deletes merged head branches — `gh repo view --json deleteBranchOnMerge`. If false, `hero-skills:ship` will clean up the remote + local branch after merge. Record as `auto-delete-branches` in HERO.md.
- Pre-commit hooks and what they run
- Linters, formatters
- Task runner (if Makefile/justfile provides commit/push/lint targets)

#### Group 2: "For planning and tracking work" (`hero-skills:plan`)
...
#### Group 3: "For testing and verification" (`hero-skills:test`)
...
#### Group 4: "For CI/CD and deployment" (`hero-skills:check`, `hero-skills:check cluster`, `hero-skills:scan`, `hero-skills:ship`)
...
- Whether to install `.github/workflows/auto-approve.yml` for `hero-skills:ship` — if absent, ask:
  *"`hero-skills:ship` lets you comment `@auto-approve` on a PR to get a Claude-verified approval (gated by self-review, no unresolved threads, and PR-metadata checks). Install `.github/workflows/auto-approve.yml`? It also requires an `ANTHROPIC_API_KEY` repo secret."*
...
FOR COMMITTING & PUSHING (hero-skills:commit, hero-skills:push)
────────────────────────────────────────────────────
...
FOR PLANNING & TRACKING (hero-skills:plan)
─────────────────────────────────────
...
FOR TESTING & VERIFICATION (hero-skills:test)
────────────────────────────────────────
...
FOR CI/CD & DEPLOYMENT (hero-skills:check, hero-skills:check cluster)
──────────────────────────────────────────────────
...
CODING CONVENTIONS (hero-skills:plan, hero-skills:commit)
──────────────────────────────────────────────
...
```markdown
# Hero Configuration
<!-- Generated by hero-skills:init. Re-run with hero-skills:init --update to refresh. -->
...
- hero-init-update: true # or false — whether hero-skills:init --update is wired into pre-commit
...
<!-- External code review bot that reviews PRs automatically.
     Used by hero-skills:respond --loop to trigger, poll, and fix feedback iteratively. -->
...
<!-- Preferred PR merge method. Default: squash. Used by hero-skills:ship
...
<!-- Whether GitHub auto-deletes merged head branches (deleteBranchOnMerge).
     If false, hero-skills:ship deletes the remote + local branch after
...
<!-- What every developer needs installed to work on this project.
     This is team-shared — individual auth/config is handled by hero-skills:setup. -->
...
<!-- Examples:
- linear (mcp__linear) — for hero-skills:plan issue management
...
How your hero skills will use this:
  hero-skills:commit    → conventional commits, pre-commit runs ruff + black + mypy
  hero-skills:push      → PRs via gh against main, link LIN-### issues
  hero-skills:plan      → fetch from Linear (mcp__linear), branch as feature/LIN-###-DESCRIPTION
  hero-skills:test      → uv sync, then ruff check + mypy + pytest, smoke at :8000
  hero-skills:check      → check GitHub Actions: ci, build, deploy
  hero-skills:check cluster    → k8s namespaces: staging, production
  hero-skills:scan    → scan pyproject.toml, check ghcr.io registry
  hero-skills:architect → single repo, Python + FastAPI, k8s deployment
  hero-skills:setup     → require node, uv, gh, docker; recommend pre-commit, linear CLI
  hero-skills:init --update → sync HERO.md via claude -p "hero-skills:init --update" in pre-commit
...
```
Run hero-skills:setup to configure your local dev environment
...
Next steps before hero-skills:ship will work:
...
Only stage the workflow file when Step 6.5 reported the file is in sync with the plugin (`INSTALL_OK=true`, covering both the fresh-install and already-up-to-date paths). When the installer returned exit 2 (`EXISTS`, drift detected), the working file is still the user's original — committing it now would falsely claim `hero-skills:init` installed the new version.
...
git commit -m "chore: initialize HERO.md and update CLAUDE.md via hero-skills:init"
```
...
```
hero-skills:init              # Investigate and create HERO.md
hero-skills:init --update     # Re-investigate and update existing config
```
