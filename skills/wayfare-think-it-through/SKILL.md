...
say the count, name `wayfare:wayfare-hero sync` as what triages it, and carry
...
entry; don't make the user re-derive them later. `wayfare:wayfare-architecture`
...
format and confirm flow), offer `wayfare:wayfare-architecture sync` to
...
`wayfare-hero do TASK_ID` later relies on, because it asks no second permission
