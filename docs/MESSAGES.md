...
(*Sending*), `wayfare:wayfare-sync-plan`'s `inbox` stage triages, wayfare-build-task's
Step 2a and `wayfare-sync-fleet` send, and the Step 0 of wayfare, wayfare-build-task and
...
**Who writes one without a person typing it.** wayfare-build-task's Step 2a, when
...
repo's roadmap: wayfare-build-task builds it, and `wayfare-sync-plan` reads it as existing
...
- **Two recipients, one message.** Two sessions in one repo both see the same
  ...
  session token and timestamp **before** doing anything, the guard wayfare-build-task's
...
- **Promoting an inbound message straight to a planned task.** That is a
  sibling writing this repo's roadmap, and `wayfare-sync-plan` will then treat the
  ground as covered.
