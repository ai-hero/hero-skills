...
check_tooling() {
  ...
  if [ -f "$ROOT/.pre-commit-config.yaml" ]; then
    if ! command -v pre-commit >/dev/null 2>&1; then
      ...
      emit BLOCKER "tooling: pre-commit CLI present but git hook not installed — Step 6 (commit) will skip every check. Run: pre-commit install"
    elif ! grep -q "generated by pre-commit" "$ROOT/.git/hooks/pre-commit" 2>/dev/null; then
      emit WARN "tooling: .git/hooks/pre-commit exists but is not pre-commit-managed — custom hook may be shadowing pre-commit checks. Verify with: pre-commit install --overwrite"
    else
      emit OK "tooling: pre-commit installed and git hook active"
    fi
  else
...
check_repo() {
  ...
  # 5. No in-progress merge / rebase / cherry-pick
  if [ -e "$ROOT/.git/MERGE_HEAD" ] \
     || [ -e "$ROOT/.git/CHERRY_PICK_HEAD" ] \
