...
#    in-session, and it prints a /goal line. Paste that; each turn builds its
#    features one after another on one branch, one commit each, tests locally,
#    and opens a single PR at the end. Work it finds along the way is absorbed
#    into the same goal rather than spawning a new one.
