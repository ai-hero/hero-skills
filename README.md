...
**`recalibrate` is on every skill that reads HERO.md.** When a skill does the
wrong thing because its config is wrong, you fix it where you noticed:
`hero-skills:ship-pr recalibrate` asks about the Repository and CI/CD fields
`ship-pr` reads, writes what you confirm, commits, and stops — it does not
then ship. `hero-skills:init-hero recalibrate` is the whole-file pass.
`scripts/hero-fields.sh SKILL` prints any skill's fields with their current
values. See [docs/RECALIBRATE.md](docs/RECALIBRATE.md).
