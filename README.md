...
| `hero-skills:architecture` | Create + converge a single root `DESIGN.md` — boundaries, dependency rules, invariants, append-only decisions; never restates what the code says. `sync` converges, `review` reports drift read-only |
| `hero-skills:wayfare` | Feature roadmap from source to the target-design repo (HERO.md-configured), two verbs: `sync` reads both ends and converges the `.plans/` roadmap; `next` advances one feature — planning via think-it-through, then straight into building via one-shot on your ready-mark. Features are SLC vertical slices (user stories, never layers) carrying subtasks, a definition of done, comments, design feedback back to the design repo, and staleness flags |
